...
export function mapIssue(row: IssueRow) {
  return {
    ...
    amplifies: row.amplifies_count ?? 0,
    ...
    petition: {
      signatures: row.petition_signatures ?? 0,
      goal: row.petition_goal ?? 0,
    },
    ...
    _amplifies_count: row.amplifies_count ?? 0,
    _petition_signatures: row.petition_signatures ?? 0,
  } as const
}
...
export function mapProfile(row: ProfileRow) {
  return {
    ...
    tier: row.tier_label ?? 'Citizen',
    meritScore: row.merit_score ?? 0,
    nextTierAt: row.next_tier_at ?? 500,
    stats: {
      reported: row.reports_count ?? 0,
      amplified: row.amplifies_count ?? 0,
